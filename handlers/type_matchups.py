...
def generate_type_loops(side_count: int = 3, type_count: int = 2):
    ...
    with open('type_loops.txt', 'w') as type_loop_file, open('perfect_loops.txt', 'w') as perfect_loop_file:
        for possible_combo in combinations_with_replacement([set(t) for n in range(type_count) for t in combinations(PokeType, n+1)], side_count):
            for combo in permutations(possible_combo, len(possible_combo)):
                if all(get_type_matchup(combo[n-1], combo[n]) > 1 for n in range(len(combo))):
